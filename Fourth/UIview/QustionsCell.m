...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]){
        ...
        [_tipButton setBackgroundImage:[UIImage imageNamed:@"icon_expand"] forState:UIControlStateNormal];
        [_tipButton setBackgroundImage:[UIImage imageNamed:@"icon_expand的副本"] forState:UIControlStateSelected];
...
-(void)setModel:(QuestionModel *)model{
    titlelabel.text =[NSString stringWithFormat:@"%@",model.help_title];
    if (model.isExpand == YES) {
        disctiplabel.hidden = NO;
        disctiplabel.text =[NSString stringWithFormat:@"%@",model.help_content];

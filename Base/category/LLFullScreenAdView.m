...
@property (nonatomic, strong) UIView *timerView;            /*!< 圆形倒计时 >*/
@property (nonatomic, weak)   CAShapeLayer *viewLayer;      //圆形路径描绘
...
- (UIButton *)skipButton{
    if (!_skipButton) {
        ...
        _skipButton.frame = CGRectMake(ScreenWidth - AdaptationWidth(74), AdaptationWidth(36), AdaptationWidth(58), AdaptationWidth(28));
        _skipButton.backgroundColor = XColorWithRBBA(34, 58, 80, 0.64);
        _skipButton.layer.cornerRadius = 2;
        _skipButton.layer.masksToBounds = YES;
        _skipButton.titleLabel.font = [UIFont fontWithName:@"PingFangSC-Regular" size:AdaptationWidth(14)];
        _skipButton.titleLabel.textAlignment = NSTextAlignmentCenter;
        [_skipButton setTitleColor:XColorWithRBBA(255, 255, 255, 1) forState:UIControlStateNormal];
...
- (void)configDefaultParameter{
    ...
    self.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight - AdaptationWidth(120));
}
...
- (void)reloadAdImageWithUrl:(NSString *)urlStr{
    ...
    if (cacheImage) {
        ...
        [manager loadImageWithURL:imageUrl options:SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
...
//    dispatch_source_set_timer(_timerWait, dispatch_walltime(NULL, 0), 0.1 * NSEC_PER_SEC, 0);
//
